...
def build_latest_trade_result_lines(journal_view: dict[str, Any]) -> list[str]:
    ...
    return [
        ...
        f"Close reason: {latest['close_reason'] or '-'}",
        f"Protection present: {'yes' if latest.get('has_initial_trade_protection') else 'no'}",
        f"Stop loss / take profit: {latest.get('stop_loss') if latest.get('stop_loss') is not None else '-'} / {latest.get('take_profit') if latest.get('take_profit') is not None else '-'}",
